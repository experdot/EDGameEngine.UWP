...
Public Class SpotPartical
    ...
    Public Sub DivideRandom(particals As List(Of SpotPartical), Optional count As Integer = 2)
        If count > 1 Then
            ...
            Else
                Size = Size * 0.9976F
                Velocity = Velocity.RotateNew(AngleOffset * 0.02F)
    ...
    Public Function GetRandomColor() As Color
        ...
        Dim r As Integer = CInt(Color.R) + Rnd.Next(-1, 2) * Rnd.Next(1, 3)
        Dim g As Integer = CInt(Color.G) + Rnd.Next(-1, 2) * Rnd.Next(1, 3)
        Dim b As Integer = CInt(Color.B) + Rnd.Next(-1, 2) * Rnd.Next(1, 3)
        If r < 0 Then r = 0
        If g < 0 Then g = 0
        If b < 0 Then b = 0
