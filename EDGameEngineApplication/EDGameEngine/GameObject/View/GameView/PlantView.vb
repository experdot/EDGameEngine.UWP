...
Friend Class PlantView
    ...
    Private Sub DrawTree(ByRef DS As CanvasDrawingSession, ByRef ParentNode As TreeNode, OffSet As Vector2, angle As Single, Optional Ratio As Single = 16)
        For Each SubNode In ParentNode.ChildNode
            SubNode.RealLoc = ParentNode.RealLoc + SubNode.Location.RotateNew(angle) * SubNode.Percent
            Dim StrokeWidth = Target.Tree.Rank * 25 * Math.Pow(Tw(SubNode.Rank), 9 - SubNode.Rank) * SubNode.Percent
            Dim BranchHeight As Single = SubNode.Location.Length * SubNode.Percent
            Dim TempV = ParentNode.RealLoc + SubNode.RealLoc.RotateNew(SubNode.DiePercent * Math.PI * 2) * (Math.Sin(SubNode.DiePercent) / 2 * 0)
            '绘制枝干
            DS.Transform = Matrix3x2.CreateRotation(Math.Atan2(SubNode.Location.Y, SubNode.Location.X) - Math.PI / 2 + Math.PI * 2 * SubNode.DiePercent * 0, TempV)
            ...
            ' DS.DrawLine(ParentNode.RealLoc, SubNode.RealLoc, Colors.White, StrokeWidth)
            '绘制花朵
