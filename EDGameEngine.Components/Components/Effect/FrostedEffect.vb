...
Public Class FrostedEffect
    ...
    ''' 磨砂半径
    ''' </summary>
    Public Property Amount As Integer = 10
    ''' <summary>
    ''' 磨砂质量
    ''' </summary>
    ...
    Public Overrides Function Effect(source As IGraphicsEffectSource, resourceCreator As ICanvasResourceCreator) As IGraphicsEffectSource
        ...
        Using bmp As CanvasBitmap = BitmapCacheHelper.CacheImage(CType(resourceCreator, CanvasDrawingSession), CType(source, ICanvasImage))
            ...
            If Quality = EffectQuality.Low Then
                For i = 0 To RawColors.Count - 1
                    Dim temp As Integer = i + Rnd.Next(0, Amount)
            ...
            Else
                ...
                For x = 0 To w - 1
                    For y = 0 To h - 1
                        tempX = x + Rnd.Next(0, Amount)
                        tempY = y + Rnd.Next(0, Amount)
